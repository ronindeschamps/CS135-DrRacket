...
(define (intersection lst1 lst2)
  (cond
    ((null? lst1) '())
    ((null? lst2) '())
    ((in-list? (car lst1) lst2)
    (make-set(cons (car lst1) (intersection (cdr lst1) lst2))))
  (else (make-set(intersection (cdr lst1) lst2)))))
...
(define (set-equal? SA SB)
 (cond
   ((null? (and SA SB))#t)
   ((equal? (intersection SA SB) (make-set SA))#t)
   (else #f)))
...
(define (set-difference LA LB)
  (cond ((or (null? LA) (null? LB)) (make-set LA))
        ((element? (car LA) LB) (make-set(set-difference (cdr LA) LB)))
        (else (cons (car LA) (make-set(set-difference (cdr LA) LB))))))
