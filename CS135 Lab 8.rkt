...
;; Pledge:I pledge my honor that I have abided by the Stevens Honor Systems.
;;----------------------------------------------------------------------------
...
(define (pred p)
  (cond ((pz? p) (eopl:error "pred error: zero has no predecessor!"))
        ((list? p) (car p))
        (else (pti (car (itp p))))))
...
(define (p- a b)
 (cond
   ((and (equal? a z) (not(equal? b z))) (eopl:error "substraction error: a < b!"))
   ((equal? b z) a)
...
(define (p= a b)
 (cond
   ((and (or (pz? a) (pz? b)) (and (pz? a) (pz? b))) #t)
   ((and (or (pz? a) (pz? b)) (not (and (pz? a) (pz? b)))) #f)
   (else (p= (pred a) (pred b)))))
...
(define (p-prime? p)
  (cond
    ((pz? p) #f)
    ((pz? (pred p)) #f)
    (else (primeHelper p (pred p)))
    ))

(define (primeHelper p s)
  (cond
    ((pz? (pred s)) #t)
    ((pz? (p% p s)) #f)
    (else (primeHelper p (pred s)))
    ))
